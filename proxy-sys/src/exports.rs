...
use thiserror::Error;
use winapi::{shared::minwindef::{HINSTANCE, FARPROC}, um::libloaderapi::{LoadLibraryA, GetProcAddress}};
...
impl ProxyDll for HINSTANCE {
    ...
    fn load_original(&self) -> Result<HINSTANCE, Box<dyn error::Error>> {
        ...
        let path = PathBuf::from_str("C:\\Windows\\System32\\")?.with_file_name(name);

        if !path.exists() {
            return Err(ExportError::LoadLibrary.into());
        }
        ...
        match path.exists() {
            ...
            false => Err(ExportError::LoadLibrary.into())
        }
    }
}
...
pub fn initialize(module: HINSTANCE) -> Result<(), Box<dyn error::Error>> {
    ...
    for (index, export) in exports.iter().enumerate() {
        let export = unsafe {
            CStr::from_bytes_with_nul_unchecked(export)
        }.as_ptr();
